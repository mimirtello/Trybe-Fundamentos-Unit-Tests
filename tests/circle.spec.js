...
describe('4 - Implemente os casos de teste para a função `circle`', () => {
  it('Verifica se ao receber um raio, a função `circle` retorna um objeto contendo os valores esperados', () => {
    ...
    expect(circle('5')).toBeUndefined(); // Teste se circle retorna undefined, caso o parâmetro passado não seja um número.
    expect(typeof(circle(1))).toEqual('object');// Teste se circle retorna um objeto.
    expect(Object.entries(circle(1)).length).toEqual(3);// Teste se o objeto retornado possui 3 propriedades.
    expect(circle()).toBeUndefined();   // Teste se a função, quando não recebe nenhum parâmetro, retorna undefined.
    expect(circle(2).circumference).toEqual(12.56);  // Teste se dentro do objeto retornado, a função retorna uma `key` com `value` igual à circunferência correta para um círculo de raio 2.
    ...
    expect(parseFloat(circle(3).area.toPrecision(4))).toEqual (28.26) &&  (parseFloat(circle(3).circumference.toPrecision(4))) .toEqual(18.84) && (parseFloat(circle(3).radius.toPrecision(4))).toEqual(3);
